...
public class Inscrire extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(temp == true || !list.isEmpty() && request.getParameter("email").matches(reg_email) ) {
			if(!list.isEmpty() && request.getParameter("email").matches(reg_email) ) {
				email = "label.eemail2";
				request.setAttribute("email", email);
			...
			Client client = new Client(request.getParameter("email").trim(),
					...
					request.getParameter("mdp").trim().hashCode());
